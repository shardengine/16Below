...
namespace Server.Mobiles
{
	...
	public class XmlSpawner : Item, ISpawner
	{
		...
		//private const double SpawnIdleTime = 72.0;              // time in hours after which idle spawns will be relocated. A value < 0 disables this feature. This does not work properly under 16Below 2.0
		...
		private static string XmlConfigsDir = "XmlSpawnerConfigs";  // default directory for loading .xml config files
			...
			foreach (SpawnObject so in m_SpawnObjects)
			{
				for (int x = 0; x < so.SpawnedObjects.Count; x++)
				{
					...
					else if (o is Mobile)
					{
						...
						// doesnt work properly under 16Below 2.0 and also doesnt properly take spawn control keywords into consideration
