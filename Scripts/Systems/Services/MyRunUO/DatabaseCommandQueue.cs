...
namespace Server.Engines.My16Below
{
    public class DatabaseCommandQueue
    {
        ...
        public DatabaseCommandQueue(string connectionString, string completionString, string threadName)
        {
            ...
            /*m_Queue.Enqueue( "DELETE FROM my16Below_characters" );
            m_Queue.Enqueue( "DELETE FROM my16Below_characters_layers" );
            m_Queue.Enqueue( "DELETE FROM my16Below_characters_skills" );
            m_Queue.Enqueue( "DELETE FROM my16Below_guilds" );
            m_Queue.Enqueue( "DELETE FROM my16Below_guilds_wars" );*/

            this.m_Sync = new ManualResetEvent(true);

            this.m_Thread = new Thread(new ThreadStart(Thread_Start));
            this.m_Thread.Name = threadName;//"My16Below Database Command Queue";
            ...
            while (true)
            {
                ...
                while (this.m_Queue.Count > 0)
                {
                        ...
                        if (obj == null)
                        {
                            if (connected)
                            {
                                if (transact != null)
                                {
                                    ...
                                    {
                                        Console.WriteLine("My16Below: Exception caught when committing transaction");
                                        Console.WriteLine(commitException);

                                        try
                                        {
                                            transact.Rollback();
                                            Console.WriteLine("My16Below: Transaction has been rolled back");
                                        }
                                        catch (Exception rollbackException)
                                        {
                                            Console.WriteLine("My16Below: Exception caught when rolling back transaction");
                            ...
                            else
                            {
                                ...
                                catch (Exception e)
                                {
                                    ...
                                    Console.WriteLine("My16Below: Unable to connect to the database");
                                    Console.WriteLine(e);
                                    this.m_HasCompleted = true;
                                    return;
                                }
                    ...
                    catch (Exception e)
                    {
                        ...
                        {
                            Console.WriteLine("My16Below: Exception caught in database thread");
                            Console.WriteLine(e);
