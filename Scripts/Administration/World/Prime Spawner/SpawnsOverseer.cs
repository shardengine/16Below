...
namespace Server.Items
{
    public class SpawnsOverseer : Item
    {
        ...
        [Constructable]
        public SpawnsOverseer()
            : this(20, 30, 5, 0)
        {
        }

        [Constructable]
        public SpawnsOverseer(int startrange)
            : this(startrange, 30, 5, 0)
        {
        }

        [Constructable]
        public SpawnsOverseer(int startrange, int startIRD, int startORD, int startOverseeing)
            : base(0x1F1E)
        {
        ...
        private void InitSeer(int startrange, int startIRD, int startORD, int startOverseeing)
        {
            ...
            Overseeing = startOverseeing; //PrimeSpawners under control
        }
        ...
        public override void GetProperties(ObjectPropertyList list)
        {
            ...
            if (m_Enable)
            {
                ...
                list.Add(1060658, "Overseeing\t{0} PrimeSpawners", Overseeing.ToString());
            }
            else
            {
        ...
        private class CheckTimer : Timer
        {
            ...
            public CheckTimer(SpawnsOverseer Crystal, TimeSpan delay)
                : base(delay, delay)
            {
        ...
        public void OnTickDoThis()
        {
            ...
            {
                List<Item> ClosePrimeSpawners = new List<Item>();
                ...
                {
                    if (item is PrimeSpawner)
                        ClosePrimeSpawners.Add(item);
                }
                if (ClosePrimeSpawners.Count > 0)
                {
                    this.Overseeing = ClosePrimeSpawners.Count;
                    ...
                    if (ClosePlayers.Count > 0) // at least one player close
                    {
                        ...
                        Restart();
                        foreach (Item pspawner in ClosePrimeSpawners) // for each PrimeSpawner close (in the List)
                        {
                            if (((PrimeSpawner)pspawner).Running == false) // if it is inactive
                            {
                                ((PrimeSpawner)pspawner).Running = true; // activate it!
                                ((PrimeSpawner)pspawner).NextSpawn = TimeSpan.FromSeconds(1); // and do a total respawn of it!
                            }
                        }
                    }
                    else if (ClosePlayers.Count <= 0) // nobody close
                    {
                        ...
                        foreach (Item pspawner in ClosePrimeSpawners) // for each PrimeSpawner close (in the List)
                        {
                            if (((PrimeSpawner)pspawner).Running == true) // if it is active
                            {
                                ((PrimeSpawner)pspawner).Running = false; // make inactive!
                ...
                else if (ClosePrimeSpawners.Count <= 0) // if SpawnsOverseer has no PrimeSpawner to seer, deactivate it
                {
                    this.Enable = false;
        ...
        public SpawnsOverseer(Serial serial)
            : base(serial)
        {
        }
