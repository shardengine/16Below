...
namespace Server.Items
{
    public class PublicMoongate : Item
    {
        ...
        public bool UseGate(Mobile m)
        {
            if (m.Criminal)
            {
                m.SendLocalizedMessage(1005561, "You are a criminal and cannot escape!", 0x22); // Thou'rt a criminal and cannot escape so easily.
                return false;
            }
            else if (SpellHelper.CheckCombat(m))
            {
                m.SendLocalizedMessage(1005564, "You are still in combat!", 0x22); // Wouldst thou flee during the heat of battle??
                return false;
            }
