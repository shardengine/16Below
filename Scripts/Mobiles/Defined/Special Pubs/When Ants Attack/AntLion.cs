    ...
    public class AntLion : BaseCreature    
	{
		...
		public AntLion() : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
                        ...
                        if ( 0.2 > Utility.RandomDouble() )
			{
                        ...
                        //    case 3: PackItem(new WarriorBones( ) );break; update needed -Fraz
                        //    case 4: PackItem(new HealerBones( ) );break; update needed -Fraz
                }
            }
		...
		{ 
			...
			if ( DateTime.UtcNow > m_NextTunnel ) 
			{            
				m_NextTunnel = DateTime.UtcNow + TimeSpan.FromSeconds( 20.0 + (10.0 * Utility.RandomDouble()) ); // 5-10 seconds 
		...
		{ 
			...
			if ( combatant == null || combatant.Deleted || combatant.Map != Map || !InRange( combatant, 12 ) || !CanBeHarmful( combatant ) || !InLOS( combatant ) ) 
				return; 
			if ( DateTime.UtcNow > m_NextPoisonSpit ) 
			{ 
				m_NextPoisonSpit = DateTime.UtcNow + TimeSpan.FromSeconds( 5.0 + (10.0 * Utility.RandomDouble()) ); // 5-15 seconds 
				this.GetDirectionTo( combatant.Location ); //check direction 
				this.Freeze( TimeSpan.FromSeconds( 2 ) ); //freeze for animation 
				this.Animate( 12, 5, 1, true, false, 0 ); //animation 
				this.PlaySound( GetAngerSound() ); // Sound 
				new APoisonTimer( this, combatant, (int)this.Hits ).Start(); // start timer with saved current hits value 
			} 
		}

        private class APoisonTimer : Timer 
		{ 
			private Mobile m_Mobile; 
			private Mobile a_Mobile; 

			public APoisonTimer( Mobile mobile, Mobile attacker, int hits ) : base( TimeSpan.FromSeconds( 1.5 ) ) 
			{ 
