...
namespace Server
{
	public static class ScriptCompiler
	{
		...
		public static string GetCompilerOptions(bool debug)
		{
			...
			AppendCompilerOption(ref sb, "/d:16Below");
        ...
        public static bool CompileLConfigurationScripts(out Assembly assembly)
        {
            return CompileConfigurationScripts(false, true, out assembly);
        }

        public static bool CompileConfigurationScripts(bool debug, out Assembly assembly)
        {
            return CompileConfigurationScripts(debug, true, out assembly);
        }

        public static bool CompileConfigurationScripts(bool debug, bool cache, out Assembly assembly)
        {
            string[] files = GetScripts("*.cs", "Config");
            ...
            if (File.Exists("Output/Config.dll"))
            {
                if (cache && File.Exists("Output/Config.hash"))
                {
                    try
                    {
                        byte[] hashCode = GetHashCode("Output/Config.dll", files, debug);

                        using (FileStream fs = new FileStream("Output/Config.hash", FileMode.Open, FileAccess.Read, FileShare.Read))
                        {
            ...
            */
            ...
            {
                string path = GetUnusedPath("Config");
...
#else
				parms.CompilerOptions = String.Format( "{0} /nowarn:169,219,414 /recurse:Config/*.cs", parms.CompilerOptions );
                ...
                if (cache && Path.GetFileName(path) == "Config.dll")
                {
                    try
                    {
                        byte[] hashCode = GetHashCode(path, files, debug);

                        using (FileStream fs = new FileStream("Output/Config.hash", FileMode.Create, FileAccess.Write, FileShare.None))
                        {
		...
		public static bool CompileCSScripts(bool debug, bool cache, out Assembly assembly)
		{
            ...
            */
			...
			if (File.Exists("Output/Scripts.dll"))
			{
				if (cache && File.Exists("Output/Scripts.hash"))
				{
                    try
					{
						var hashCode = GetHashCode("Output/Scripts.dll", files, debug);

						using (
							FileStream fs = new FileStream("Output/Scripts.hash", FileMode.Open, FileAccess.Read, FileShare.Read))
						{
							using (BinaryReader bin = new BinaryReader(fs))
							{
       ...
       //     Console.WriteLine("Compiling .CS Scripts... (Scripts)");
					...
					try
					{
						...
						using (
							FileStream fs = new FileStream(
								"Output/Scripts.hash", FileMode.Create, FileAccess.Write, FileShare.None))
						{
							using (BinaryWriter bin = new BinaryWriter(fs))
							{
								bin.Write(hashCode, 0, hashCode.Length);
		...
		//	Console.Write(".Compiling VB.NET scripts...");
			...
			if (File.Exists("Output/Scripts.VB.dll"))
			{
				...
				{
					var hashCode = GetHashCode("Output/Scripts.VB.dll", files, debug);

					try
					{
						using (
							FileStream fs = new FileStream("Output/Scripts.VB.hash", FileMode.Open, FileAccess.Read, FileShare.Read))
						{
							using (BinaryReader bin = new BinaryReader(fs))
							{
								var bytes = bin.ReadBytes(hashCode.Length);
			...
			using (VBCodeProvider provider = new VBCodeProvider())
			{
				...
				if (cache && Path.GetFileName(path) == "Scripts.VB.dll")
				{
					try
					{
						...
						using (
							FileStream fs = new FileStream(
								"Output/Scripts.VB.hash", FileMode.Create, FileAccess.Write, FileShare.None))
						{
							using (BinaryWriter bin = new BinaryWriter(fs))
							{
								bin.Write(hashCode, 0, hashCode.Length);
		...
		{
			string path = Path.Combine(Core.BaseDirectory, String.Format("Output/{0}.dll", name));

			for (int i = 2; File.Exists(path) && i <= 1000; ++i)
			{
				path = Path.Combine(Core.BaseDirectory, String.Format("Output/{0}.{1}.dll", name, i));
			}
		...
		public static void DeleteFiles(string mask)
		{
			try
			{
				var files = Directory.GetFiles(Path.Combine(Core.BaseDirectory, "Output"), mask);
		...
		public static bool Compile(bool debug, bool cache)
		{
            ...
            if (CompileConfigurationScripts(debug, cache, out assembly))
            {
       ...
       //     string text = "...................";
        ...
        public static string[] GetScripts(string filter, string path)
        {
            ...
            GetScripts(list, Path.Combine(Core.BaseDirectory, String.Format(@"..\{0}", path)), filter);
    ...
    public static string[] GetScripts(string filter)
    {
        ...
        //   Console.WriteLine("PATH {0}", Path.Combine(Core.BaseDirectory, @"..\Scripts"));
    ...
    public static void GetScripts(List<string> list, string path, string filter)
    {
        ...
        //    Console.WriteLine("Compiling {0}", Directory.GetFiles(path, filter));
